...
def primary_suggested_plans():
    ...
    data = serialize_cursor_response(plan_collection.find({"author_email": {"$ne":email},"major": user["major"], "school": user["school"]}).limit(5))
    ...
    if len(data) == 0:
        ...
        data = serialize_cursor_response(plan_collection.find({"author_email": {"$ne":email},"school": user["school"]}).limit(3))
        response_data["plans"] = data
...
def search_plans():
    ...
    if school_filter and major_filter:
        major_filter = major_filter.replace("!@"," ")
        major_filter = major_filter.replace("!~","&")
...
@app.route("/remove_comment/<plan_id>/<comment_id>", methods=["DELETE"])
@cross_origin(origin='*',headers=['Content-Type','Authorization'])
def remove_comment(plan_id, comment_id):
